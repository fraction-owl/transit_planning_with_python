...
Reads a polygon layer of zones (for example, one polygon around each transit
center) and a GTFS feed, finds every stop inside or on the edge of each zone,
and logs the stop_id lists as text to paste into the CONFIGURATION sections of
...
``requirements.txt``). Paste each block over the matching setting, then add
capacities.
"""
...
# Zone polygons, e.g. one around each transit center (any format geopandas
# reads). The layer needs a CRS; stops are projected into it.
ZONES_PATH: str = r"Path\To\Your\Zones.shp"

# Optional attribute that names each zone, e.g. "NAME". Polygons that share a
# name form one zone. Leave "" to number zones by position in the file ("Zone 1"
...
# DE-9IM pattern for two polygons whose interiors share area (touching is not enough).
...
def load_zones(zones_path: str, name_field: str = "") -> gpd.GeoDataFrame:
    """Read the zone polygons, check them, and name each one.

    Args:
        zones_path: Polygon layer in any format geopandas reads.
        name_field: Attribute naming each zone, or "" to number zones by
            position in the file.

    Returns:
        One row per polygon with ``polygon`` (its 1-based position in the
        file), ``zone`` (its name) and ``geometry``, in the layer's CRS.

    Raises:
        OSError: *zones_path* does not exist.
        ValueError: The layer cannot be read, is empty, has no CRS or lacks
            *name_field*; a polygon is missing, not a polygon or invalid; a
            longitude/latitude layer has coordinates out of range; or an
            unnamed polygon's number is another polygon's name.
    """
    if not os.path.exists(zones_path):
        raise OSError(f"Zone layer not found: {zones_path}")
    try:
        layer = gpd.read_file(zones_path)
    ...
    if layer.crs is None:
        raise ValueError(
            f"Zone layer '{zones_path}' has no CRS (missing or unknown .prj). "
            "Define its projection before running."
        )
    if name_field and name_field not in layer.columns:
        fields = sorted(str(column) for column in layer.columns if column != "geometry")
        raise ValueError(
    ...
    numbers = list(range(1, len(layer) + 1))
    _check_polygons(list(layer.geometry), numbers)
    if layer.crs.is_geographic:
        ...
        if west < -180 or east > 180 or south < -90 or north > 90:
            raise ValueError(
                ...
                f"({layer.crs.to_string()}), but its coordinates are out of range. The file "
                "probably holds projected coordinates without saying so; export it again with "
                "its CRS."
            )
    names = _zone_names(list(layer[name_field]) if name_field else None, numbers)
    zones = gpd.GeoDataFrame(
        {"polygon": numbers, "zone": names}, geometry=list(layer.geometry), crs=layer.crs
    )
    for zone, count in zones["zone"].value_counts(sort=False).items():
        if count > 1:
            logging.info("Zone '%s' combines %d polygons.", zone, count)
    logging.info("Read %d zone(s) from %d polygon(s).", zones["zone"].nunique(), len(zones))
    return zones


def _check_polygons(geometries: List[Any], numbers: List[int]) -> None:
    """Raise ValueError naming zone polygons that are missing, not polygons, or invalid."""
    missing = [n for n, geom in zip(numbers, geometries) if geom is None or geom.is_empty]
    if missing:
        raise ValueError(f"Zone polygon(s) {_preview(missing)} have no geometry.")
    wrong = [
        f"{n} ({geom.geom_type})"
        for n, geom in zip(numbers, geometries)
        if geom.geom_type not in {"Polygon", "MultiPolygon"}
    ]
    if wrong:
        raise ValueError(f"Zones must be polygons; found {_preview(wrong)}.")
    invalid = [
        f"{n} ({shapely.is_valid_reason(geom)})"
        for n, geom in zip(numbers, geometries)
        if not geom.is_valid
    ]
    if invalid:
        raise ValueError(
            f"Invalid zone polygon(s) {_preview(invalid)}. Repair them (for example with "
            "ArcGIS Repair Geometry or QGIS Fix Geometries) and rerun."
        )
...
def check_zone_overlaps(zones: gpd.GeoDataFrame) -> None:
    ...
    """
    shapes = {
        str(zone): group.geometry.union_all() for zone, group in zones.groupby("zone", sort=False)
    }
    names = list(shapes)
    geometries = [shapes[name] for name in names]
    left, right = shapely.STRtree(geometries).query(geometries, predicate="intersects")
    candidates = sorted({(min(i, j), max(i, j)) for i, j in zip(left.tolist(), right.tolist())})
    overlaps = [
        f"'{names[i]}' and '{names[j]}'"
        for i, j in candidates
        if i != j and geometries[i].relate_pattern(geometries[j], SHARED_AREA_PATTERN)
    ]
    if overlaps:
        raise ValueError(
...
def assign_stops_to_zones(stops: pd.DataFrame, zones: gpd.GeoDataFrame) -> pd.DataFrame:
    """Find the zone each stop falls in, counting a stop on a zone's edge as inside.

    Stops are projected into the zone layer's CRS. Zones without stops are
    ...
    Returns:
        ...
        zones in file order and stop_ids in natural order within each zone.

    Raises:
        ValueError: No stop falls in any zone, or a stop lies on the shared
            edge of two zones.
    """
    points = gpd.GeoDataFrame(
        stops[["stop_id", "stop_code", "stop_name"]].copy(),
        geometry=gpd.points_from_xy(stops["stop_lon"], stops["stop_lat"]),
        crs="EPSG:4326",
    ).to_crs(zones.crs)
    joined = gpd.sjoin(points, zones[["zone", "geometry"]], how="inner", predicate="intersects")
    pairs = pd.DataFrame(joined[ASSIGNMENT_COLUMNS]).drop_duplicates(["zone", "stop_id"])
    if pairs.empty:
        raise ValueError(
            "No stop falls inside any zone. Check that the zones cover the stops and that "
            "the zone layer's CRS is right (a wrong CRS puts the polygons somewhere else)."
        )
...
def build_config_text(
    ...
    header = [
        rule,
        "# Cluster stop lists from cluster_stops_from_zones_gpd.py",
        f"# Zones: {zones_path}",
        f"# GTFS:  {gtfs_path}",
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--zones-path",
        default=ZONES_PATH,
        help="Zone polygon layer (any format geopandas reads).",
    )
    parser.add_argument(
        "--zone-name-field",
