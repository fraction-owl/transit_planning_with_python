"""Profile how redundant each transit route is, from several angles, per schedule calendar (ArcPy).

ArcPy port of ``route_redundancy_profile_gpd.py`` for environments running
ArcGIS Pro's bundled Python (no geopandas/shapely). The method, filters,
schedule classification, and output tables are identical; only the spatial
plumbing differs — stops are projected and buffered with ``arcpy`` geometry
objects instead of shapely, and the walk-distance neighbor search uses
``scipy.spatial.cKDTree`` (bundled with Pro).
...
1. **Shared vs. solo stops** — a route's stop is *shared* when any other route
   ...
   no other route comes that close.
...
3. **Shared timepoints** — the walking-distance test again, restricted to
   ...
   fall back to every timed stop). Two routes whose timepoints coincide serve
...
4. **Solo vs. shared service area** — a route's service area is the union of
   ...
   service area.
5. **Paired routes** — a one-way couplet, or a loop published as two
   direction-named routes, reads as near-total redundancy but is really one
   service split by direction. Pairs declared in ``ROUTE_PAIRS`` are excluded
   from each other's metrics. Undeclared pairs whose service areas mutually
   overlap by at least ``PAIRED_ROUTE_MIN_OVERLAP_PCT`` (default 90%) are
   *suggested* in a ``suggested_pairs`` column and a warning log; suggestions
   alone never change any number.
...
exceptions) into Weekday / Saturday / Sunday / Holiday. Feeds with no usable
calendar files are profiled once under an "All Service" label.

Caveats: distances are straight-line in a projected spatial reference (no
pedestrian network), service areas are stop buffers (not parcels reachable on
foot), and the timed-alternative check asks "was another bus coming soon
nearby", not "does the other route continue to my destination".

Inputs
------
- A GTFS feed (folder or .zip) with stops.txt, routes.txt, trips.txt, and
  stop_times.txt. calendar.txt / calendar_dates.txt, when present, drive the
  per-schedule classification.

Outputs
-------
- ``route_redundancy_profile.csv``: one row per (schedule, route) with
  shared/solo stop counts, routes within walking distance, timed alternatives,
  shared timepoint counts, total/solo/shared service area in square miles,
  declared paired routes, and suggested pairs.
- ``route_redundancy_partners.csv`` (optional): one row per
  (schedule, route, partner route) pair with shared stop and timepoint counts,
  the nearest walk distance, the shortest feasible wait, the service-area
  overlap percentage, and the paired/suggested flags.
...
and run from a shell, ArcGIS Pro's Python window, or a Jupyter notebook using
ArcGIS Pro's bundled Python.

Requires
--------
ArcGIS Pro (arcpy) plus pandas, numpy, and scipy (all bundled with Pro).
"""
...
# --- Paired routes ------------------------------------------------------------
# Known direction-split pairs — one-way couplets, or a loop published as two
# direction-named routes. Declare each pair once, by route_id or
# route_short_name, e.g. [("99A", "99B")]. Declared partners stop counting as
# "another route" in each other's metrics (shared stops, routes within walk,
# timed alternatives, timepoints, shared area): a couplet is one service split
# by direction, not redundancy. Its metrics are then measured against the rest
# of the network.
ROUTE_PAIRS: list[tuple[str, str]] = []

# Undeclared route pairs whose service areas mutually overlap by at least this
# percentage are *suggested* in the output (``suggested_pairs`` column and a
# warning log) so they can be reviewed and added to ROUTE_PAIRS. Suggestions
# never change any metric — only declared pairs do.
...
# Projected spatial reference (WKID) used for distance and area math. The
# default is NAD83 / Conterminous US Albers (EPSG:5070), an equal-area
# projection in meters that is fine for walking distances anywhere in the US.
# For non-US feeds, set a local projected WKID. Web Mercator (3857) is
# accepted; its latitude-dependent scale distortion is corrected from the mean
# stop latitude so buffers and areas stay true to the ground.
PROJECTED_CRS_WKID: int = 5070
...
# -----------------------------------------------------------------------------
# Spatial reference plumbing (the part that differs from the geopandas twin)
# -----------------------------------------------------------------------------


def make_spatial_reference(wkid: int) -> arcpy.SpatialReference:
    """Build the projected spatial reference used for all distance/area math.

    Args:
        wkid: Well-known ID of a *projected* spatial reference.

    Returns:
        The ``arcpy.SpatialReference`` for *wkid*.

    Raises:
        ValueError: If arcpy does not recognize *wkid*, or it is not a
            projected spatial reference (geographic degrees cannot measure
            walking distances or square miles).
    """
    try:
        target_sr = arcpy.SpatialReference(wkid)
    except (RuntimeError, ValueError, arcpy.ExecuteError) as exc:
        raise ValueError(
            f"PROJECTED_CRS_WKID {wkid!r} is not a spatial reference arcpy recognizes: {exc}"
        ) from exc
...
def ground_meters_per_unit(
    target_sr: arcpy.SpatialReference,
    mean_latitude: Optional[float] = None,
) -> float:
    """Return how many *ground* meters one linear unit of *target_sr* spans.

    For most projected spatial references this is simply the unit's length
    (1.0 for meters, 0.3048 for US feet). Web Mercator (WKID 3857 / 102100)
    inflates distances by roughly ``1/cos(latitude)`` — at 39°N a true quarter
    mile spans ~1.29x as many map "meters" — so when *target_sr* is Web
    Mercator and *mean_latitude* is supplied, the factor is scaled down
    accordingly. Every distance and area in this script goes through this one
    factor, so buffers, walk distances, and square miles all stay true to the
    ground.

    Args:
        target_sr: Projected spatial reference used for geometry operations.
        mean_latitude: Mean WGS 84 latitude of the stops, used to correct Web
            Mercator's scale distortion. Ignored for other spatial references.

    Returns:
        Ground meters per linear unit of *target_sr*.
    """
    meters_per_unit = float(target_sr.metersPerUnit or 1.0)
    if target_sr.factoryCode in (3857, 102100):
        if mean_latitude is not None and -89.0 < mean_latitude < 89.0:
            ...
            logging.info(
                ...
                "distances and areas to preserve ground measurements.",
                1.0 / scale,
                mean_latitude,
            )
        logging.warning(
            "Web Mercator is not equal-area; service-area square miles are approximate. "
            "Prefer an equal-area projection such as EPSG:5070 for area math."
        )
    logging.debug(
        "One %s of the spatial reference spans %.4f ground meters.",
...
def mean_stop_latitude(stops: pd.DataFrame) -> Optional[float]:
    """Return the mean numeric ``stop_lat`` of *stops*, or ``None`` if none parse."""
    lat_values = pd.to_numeric(stops["stop_lat"], errors="coerce").dropna()
    return float(lat_values.mean()) if not lat_values.empty else None
...
def project_stops(
    ...
    """Project stops into *target_sr* and attach coordinate columns.

    Two coordinate pairs are attached. ``sr_x`` / ``sr_y`` are in the linear
    units of *target_sr* and feed the arcpy geometry work (buffers, areas).
    ``x`` / ``y`` are the same positions expressed in ground meters (scaled by
    *meters_per_unit*) and feed the walk-distance neighbor search, so the
    proximity math reads exactly like the geopandas twin.

    Args:
        stops: Parsed *stops.txt* with ``stop_id``, ``stop_lat``, ``stop_lon``.
        target_sr: Projected spatial reference for all distance and area math.
        meters_per_unit: Output of :func:`ground_meters_per_unit`.

    Returns:
        A DataFrame of unique stops with ``sr_x``, ``sr_y``, ``x``, and ``y``.
    ...
    sr_x: list[float] = []
    sr_y: list[float] = []
    for lon, lat in zip(unique["stop_lon"], unique["stop_lat"]):
        point = arcpy.PointGeometry(arcpy.Point(float(lon), float(lat)), wgs84_sr)
        projected = point.projectAs(target_sr).firstPoint
        sr_x.append(float(projected.X))
        sr_y.append(float(projected.Y))
    unique["sr_x"] = sr_x
    unique["sr_y"] = sr_y
    unique["x"] = unique["sr_x"] * meters_per_unit
    unique["y"] = unique["sr_y"] * meters_per_unit
    return unique
...
def compute_pair_stats(
    ...
    *check_times* is on, each stop pair is also tested for a catchable
    ...
    Args:
        stops_df: Projected stops with ``stop_id``, ``x``, ``y`` (ground
            meters, from :func:`project_stops`).
    ...
    """
    sub = stops_df.loc[stops_df["stop_id"].isin(set(serves))]
    if sub.empty:
        return {}
    coords = np.column_stack([sub["x"].to_numpy(dtype=float), sub["y"].to_numpy(dtype=float)])
...
def build_route_buffers(
    ...
    """Buffer each route's stops into one dissolved service-area polygon.

    Buffering an ``arcpy.Multipoint`` of the route's stops yields a single
    (possibly multipart) polygon — the union of the per-stop circles — in one
    geometry call.

    Args:
        stops_df: Projected stops with ``stop_id``, ``sr_x``, ``sr_y`` (from
            :func:`project_stops`).
        route_stops: Mapping of route_id → the stop_ids it serves.
        radius_units: Buffer radius in the linear units of *target_sr*.
        target_sr: Spatial reference the ``sr_x`` / ``sr_y`` columns are in.

    Returns:
        Mapping of route_id → its service-area ``arcpy.Polygon``, or ``None``
        when none of the route's stops have usable coordinates.
    """
    xy = {
        stop_id: (float(x), float(y))
        for stop_id, x, y in zip(stops_df["stop_id"], stops_df["sr_x"], stops_df["sr_y"])
    }
    buffers: dict[str, Any] = {}
    for route_id, stop_ids in route_stops.items():
        points = [arcpy.Point(*xy[s]) for s in sorted(stop_ids) if s in xy]
        if not points:
            buffers[route_id] = None
            continue
        multipoint = arcpy.Multipoint(arcpy.Array(points), target_sr)
        buffers[route_id] = multipoint.buffer(radius_units)
    return buffers


def _geometry_area(geom: Any) -> float:
    """Return the area of an arcpy geometry, treating ``None``/empty as zero."""
    if geom is None:
        return 0.0
    try:
        return float(geom.area or 0.0)
    except (AttributeError, TypeError):
        return 0.0


def _extents_overlap(a: Any, b: Any) -> bool:
    """Cheap bounding-box test used to skip geometry calls that cannot intersect."""
...
def compute_service_areas(
    ...
    declared_pairs: Optional[set[frozenset[str]]] = None,
    meters_per_unit: float = 1.0,
    ...
    service area; shared is the difference. A route's declared pair partner
    is not subtracted — the couplet's other half is the same service, so it
    cannot make coverage "shared".

    Args:
        buffers: Output of :func:`build_route_buffers`.
        declared_pairs: Output of :func:`resolve_route_pairs`; ``None`` means
            no pairs are declared.
        meters_per_unit: Output of :func:`ground_meters_per_unit`, used to
            convert the spatial reference's square units to square miles.

    Returns:
        Mapping of route_id → ``(total_sqmi, solo_sqmi, shared_sqmi)``.
    """
    pairs_declared = declared_pairs or set()
    sqmi_per_square_unit = meters_per_unit**2 / _SQM_PER_SQMI

    areas: dict[str, tuple[float, float, float]] = {}
    for route_id, geom in buffers.items():
        if _geometry_area(geom) <= 0.0:
            areas[route_id] = (0.0, 0.0, 0.0)
            continue
        solo = geom
        for other_id, other in buffers.items():
            if other_id == route_id or _geometry_area(other) <= 0.0:
                continue
            if frozenset((route_id, other_id)) in pairs_declared:
                continue
            if not _extents_overlap(geom.extent, other.extent):
                continue
            solo = solo.difference(other)
            if _geometry_area(solo) <= 0.0:
                break
        total_sqmi = _geometry_area(geom) * sqmi_per_square_unit
        solo_sqmi = _geometry_area(solo) * sqmi_per_square_unit
    ...
    *both* directions, while an ordinary crossing or branching route covers
    ...
    for route_id, geom in buffers.items():
        ...
        for other_id, other in buffers.items():
            ...
            if not _extents_overlap(geom.extent, other.extent):
                continue
            inter = _geometry_area(geom.intersect(other, 4))
            if inter > 0.0:
                pcts[(route_id, other_id)] = 100.0 * inter / area
    return pcts
...
def resolve_route_pairs(
    ...
    """Resolve declared pair tokens to canonical route_id pairs.

    Each token matches against ``route_id`` OR ``route_short_name`` so users
    can list whichever identifier they know. A short name that maps to several
    route_ids pairs every combination. A pair with an unmatched token is
    skipped with a warning rather than failing the run.

    Args:
        pairs: Two-token sequences, e.g. ``[("99A", "99B")]``.
        routes: Parsed *routes.txt*.

    Returns:
        Set of two-element ``frozenset``s of route_id strings.

    Raises:
        ValueError: If an entry does not contain exactly two tokens.
    """
    ids = {str(r) for r in routes["route_id"]}
    short_to_ids: dict[str, set[str]] = {}
    if "route_short_name" in routes.columns:
        for rid, short in zip(routes["route_id"], routes["route_short_name"].fillna("")):
            token = str(short).strip()
            if token:
                short_to_ids.setdefault(token, set()).add(str(rid))

    def _match(token: str) -> set[str]:
        text = str(token).strip()
        return {text} if text in ids else short_to_ids.get(text, set())

    declared: set[frozenset[str]] = set()
    for pair in pairs:
        if len(pair) != 2:
            raise ValueError(
                f"Route pairs must name exactly two routes (route_id or route_short_name); "
                f"got {tuple(pair)!r}."
            )
        matches = [_match(token) for token in pair]
        unmatched = [str(token) for token, found in zip(pair, matches) if not found]
        if unmatched:
            logging.warning(
                "Declared route pair %r: no route matches %s — pair ignored.",
                tuple(pair),
                ", ".join(unmatched),
            )
            continue
        for id_a in matches[0]:
            for id_b in matches[1]:
                if id_a != id_b:
                    declared.add(frozenset((id_a, id_b)))
    if declared:
        logging.info("Declared route pair(s) resolved: %d.", len(declared))
    return declared
...
def build_profile_tables(
    ...
    Declared pair partners are excluded from each other's redundancy metrics
    (shared stops, routes within walk, timed alternatives, timepoint sharing)
    — a couplet is one service split by direction, so its other half is not
    competition. Undeclared partners whose service areas mutually overlap by
    at least *paired_min_overlap_pct* are surfaced as suggestions instead.
    ...
    Args:
        ...
        declared_pairs: Output of :func:`resolve_route_pairs` — the declared
            direction-split pairs. ``None`` means none are declared.
        paired_min_overlap_pct: Mutual service-area overlap (percent) at or
            above which an undeclared pair is suggested.
    ...
    for route_id in sorted(route_stops, key=_label):
        ...
        def _is_pair(partner_id: str, me: str = route_id) -> bool:
            return frozenset((me, partner_id)) in pairs_declared

        all_ids = sorted(route_partners, key=_label)
        # Declared partners count whenever they run this schedule, walkable or not.
        paired_ids = sorted((p for p in route_stops if _is_pair(p)), key=_label)
        walk_ids = [p for p in all_ids if not _is_pair(p)]
        timed_ids = [p for p in walk_ids if route_partners[p].timed_feasible]

        shared_stops: set[str] = set()
        for partner_id in walk_ids:
            shared_stops |= route_partners[partner_id].shared_stops
        n_shared = len(shared_stops)

        n_timepoints = len(tp_route_stops.get(route_id, set()))
        route_tp_partners = tp_partners.get(route_id, {})
        tp_ids = [p for p in sorted(route_tp_partners, key=_label) if not _is_pair(p)]
        shared_timepoints: set[str] = set()
        for partner_id in tp_ids:
            shared_timepoints |= route_tp_partners[partner_id].shared_stops
        ...
        summary_rows.append(
            {
                ...
                "n_timepoint_partner_routes": len(tp_ids),
                "timepoint_partner_routes": ", ".join(_label(p) for p in tp_ids),
                ...
                "n_suggested_pairs": len(suggested_ids),
                "suggested_pairs": ", ".join(_label(p) for p in suggested_ids),
            }
        )

        for partner_id in all_ids:
            ...
            detail_rows.append(
                {
                    ...
                    "suggested_pair": partner_id in suggested_ids,
                }
            )
...
def write_run_log(output_dir: Path, summary_lines: List[str]) -> bool:
    ...
    lines: List[str] = [
        "=" * 72,
        "ROUTE REDUNDANCY PROFILE (ARCPY) RUN LOG",
        "=" * 72,
...
def run(
    ...
    routes_exclude: Sequence[str] = (),
    service_ids: Sequence[str] = (),
    projected_wkid: int = PROJECTED_CRS_WKID,
    write_detail: bool = WRITE_DETAIL,
    route_pairs: Sequence[Sequence[str]] = (),
    paired_min_overlap_pct: float = PAIRED_ROUTE_MIN_OVERLAP_PCT,
) -> pd.DataFrame:
    ...
    Args:
        ...
        projected_wkid: WKID of the projected spatial reference for distance
            and area math.
        write_detail: Also write the per-pair partner detail CSV.
        route_pairs: Declared direction-split pairs, two tokens each
            (route_id or route_short_name); their partners are excluded from
            each other's redundancy metrics.
        paired_min_overlap_pct: Mutual service-area overlap (percent) at or
            above which an undeclared pair is suggested.
    ...
    Raises:
        ...
        ValueError: The feed fails column validation or cannot be parsed, a
            route-pair entry does not contain exactly two tokens, or
            *projected_wkid* is not a projected spatial reference.
    ...
    declared_pairs = resolve_route_pairs(route_pairs, data["routes"])

    stops = filter_platform_stops(data["stops"]) if FILTER_TO_PLATFORM_STOPS else data["stops"]
    target_sr = make_spatial_reference(projected_wkid)
    meters_per_unit = ground_meters_per_unit(target_sr, mean_stop_latitude(stops))
    stops_df = project_stops(stops, target_sr, meters_per_unit)
    known_stop_ids = set(stops_df["stop_id"])

    radius_m = distance_to_meters(walk_distance, walk_distance_unit)
    radius_units = radius_m / meters_per_unit
    logging.info(
        "Walking distance: %.0f m (%.2f %s in %s). "
        "Timed-alternative check: %s (max wait %.0f min).",
        radius_m,
        radius_units,
        target_sr.linearUnitName,
        target_sr.name,
    ...
    for label in ordered_labels:
        ...
        tp_pair_stats = compute_pair_stats(stops_df, tp_serves, radius_m, check_times=False)

        route_stops: dict[str, set[str]] = {}
        for stop_id, route_ids in serves.items():
            for route_id in route_ids:
                route_stops.setdefault(route_id, set()).add(stop_id)
        buffers = build_route_buffers(stops_df, route_stops, radius_units, target_sr)
        areas = compute_service_areas(buffers, declared_pairs, meters_per_unit)
    ...
    for row in summary.itertuples():
        if str(row.suggested_pairs):
            me = str(row.route_short_name) or str(row.route_id)
            for other in str(row.suggested_pairs).split(", "):
                suggestions.add(tuple(sorted((me, other))))
    if suggestions:
        logging.warning(
            "%d undeclared route pair(s) look like one service split by direction "
            "(>= %g%% mutual service-area overlap): %s. If so, declare them in "
            'ROUTE_PAIRS (or --route-pairs "A:B") and rerun so each partner stops '
            "counting toward the other's redundancy.",
            len(suggestions),
            paired_min_overlap_pct,
            "; ".join(f"{a} + {b}" for a, b in sorted(suggestions)),
        )
    ...
    summary_lines = [
        ...
        f"Spatial reference:    {target_sr.name} (WKID {projected_wkid})",
        ...
        f"Declared route pairs: {len(declared_pairs)}",
        f"Pair suggestions:     >= {paired_min_overlap_pct:g}% mutual service-area overlap",
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "ArcPy edition for ArcGIS Pro's bundled Python."
        ),
    ...
    parser.add_argument(
        "--route-pairs",
        nargs="*",
        default=[f"{a}:{b}" for a, b in ROUTE_PAIRS],
        help=(
            "Declared direction-split pairs as ROUTE_A:ROUTE_B tokens (route_id or "
            "route_short_name), e.g. 99A:99B. Partners are excluded from each other's "
            "redundancy metrics."
        ),
    )
    parser.add_argument(
        ...
        help="Mutual service-area overlap (percent) at or above which undeclared pairs are "
        "suggested.",
    )
    parser.add_argument(
        "--no-time-check",
    ...
    parser.add_argument(
        ...
        help="WKID of the projected spatial reference used for distance and area math.",
    )
    return parser
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    route_pairs: list[tuple[str, str]] = []
    for token in args.route_pairs:
        parts = [part.strip() for part in str(token).split(":")]
        if len(parts) != 2 or not all(parts):
            logging.warning(
                "--route-pairs entries must look like ROUTE_A:ROUTE_B (route_id or "
                "route_short_name); got %r.",
                token,
            )
            return 2
        route_pairs.append((parts[0], parts[1]))
