...
Turn NTD-format monthly summaries into route-level trend reports, process ridership by stop, and join ridership figures to stop geometries. Load factor monitoring is also included, built on ridecheck outputs — the data shape varies between agencies, but the underlying structure is robust and adaptable. For agencies without vendor ridecheck software, two TIDES-derived tools build the inputs the rest of this folder expects directly from raw stop-event data: `ridership_from_tides.py` sums boardings/alightings, and `max_load_from_tides.py` derives each trip's peak load — both at a configurable time-period × month grain, mirroring the TIDES OTP calculator. Before any stop-level ridership export is trusted, `ridership_gtfs_coverage.py` vets it against the GTFS feed day type by day type (Weekday/Saturday/Sunday): it flags scheduled stops with no ridership record at all — sorted by scheduled service intensity, since a 40-trips-a-day stop with no record points at the data pipeline while a twice-a-day stop is probably just unused — and, in the other direction, ridership records at stops or on day types the feed doesn't schedule, catching stale stop IDs and broken counting pipelines before they contaminate an analysis.
...
Stop guessing where buses are getting delayed. Pivot route-level OTP trends, diagnose runtime by trip event, and analyze segment-level runtimes. Roll on-time performance (`otp_by_route.py`) and running time (`route_runtime_tides.py`) up to the route over a trailing window for demand modeling. A stop-level counterpart (`tides_stop_otp_flagger.py`) inverts the route view and flags individual stops whose AVL data coverage or OTP falls well below the baselines of *multiple* routes serving them — a signature of stop-specific operational or data issues (mis-placed geofences, relocated stops, chronic congestion) that route rollups average away. Two GTFS-anchored coverage validators close that flagger's documented blind spot — an observed-only export can't reveal what never appears in it: `trip_coverage_tides.py` reports, per scheduled GTFS trip and day type, the share of scheduled dates that actually show up in `trips_performed` (separating never-recorded trips from dispatch cancellations), and `stop_coverage_tides.py` compares each stop's expected trip-visits from `stop_times` against `stop_visits`, decomposing every gap into trip-level dropouts versus stop-level ones (the geofence/stop-matching signature) and flagging invisible and low-coverage stops. Both treat the GTFS↔TIDES trip-ID join rate as a headline diagnostic, so a feed from the wrong signup can't masquerade as missing data. Includes CleverWorks-to-TIDES converters for stop visits and trips performed.
