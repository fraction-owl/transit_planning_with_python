...
Stop guessing where buses are getting delayed. Pivot route-level OTP trends, diagnose runtime by trip event, and analyze segment-level runtimes. Roll on-time performance (`otp_by_route.py`) and running time (`runtime_by_route.py`) up to the route over a trailing window for demand modeling. A stop-level counterpart (`otp_by_stop.py`) inverts the route view — computing OTP and AVL data coverage per stop and natively flagging stops that fall well below the baselines of *multiple* routes serving them — a signature of stop-specific operational or data issues (mis-placed geofences, relocated stops, chronic congestion) that route rollups average away. A trip and time-of-day view (`otp_by_trip_and_hour.py`) scores TIDES `stop_visits` per scheduled trip and per hour of the service day (owl trips past midnight stay on their operational date), split into configurable day-type buckets — Weekday / Saturday / Sunday by default — with optional route filtering. Includes CleverWorks-to-TIDES converters for stop visits and trips performed.
*Folder:* `operations_tools/`

### Service Coverage & Catchments *(arcpy + GeoPandas)*
Generate GTFS-to-Census catchment areas and calculate service coverage by district — built for **Title VI** equity/demographic analysis and service planning. Site-to-route proximity tools support site planning collaboration with developers, elected officials, and partner agencies. A private-shuttle tool (`private_shuttle_coverage_by_route_gpd.py`, with an `_arcpy` twin for ArcGIS Pro environments) turns the hand-maintained operator spreadsheets agencies actually have — partial addresses, partial coordinates, free-text notes — into a clean registry, a geocoding worklist, a `Private_Shuttle_Stops` point layer for the strategic-site coverage counts, and an optional per-route rollup of shuttle sites (splitting transit *feeders* from potential *competitors*) that plugs into the demand-modeling bundles, so agencies can see where employer/university/residential shuttles run and test whether they move ridership. Service start/end dates are normalized and carried through, and the rollup's `--as-of` flag counts only the shuttles active on a chosen date — the groundwork for modeling dated shuttle openings and closures against the ridership panel.
...
Quantify what changes when you remove or relocate a stop. Score the downstream impact of stop removals on target routes, flag stop-spacing outliers, and compare GTFS stop sets across versions. For service changes, a companion `gtfs_route_diff.py` compares two feeds at the *route* level — classifying each route as eliminated, added, rekeyed, or changed (major/minor alignment, major/minor schedule, reblocking, name/attribute, calendar-approach, day-type, and fare changes), reconciling `route_id` renumbering so renamed routes aren't double-counted, flagging candidate route splits and merges, and reporting each feed's declared date range. Every threshold is a configurable knob. A scenario-based cut engine (`service_cut_impact_gpd.py`, with an `_arcpy` twin for ArcGIS Pro environments) quantifies what is lost when routes, trips, or stops are cut: each named scenario (whole routes, explicit trips, departure-window thinning, stop truncations, or any combination) is diffed against the baseline feed to report stops eliminated or degraded by time of day, walk-buffer coverage lost (with optional demographics), riders affected from optional ridership inputs (a trip-level table, a route×stop stop-usage export, or both), GTFS-approximated revenue hours and miles saved, and ready-to-map shapefiles (remaining and removed route alignments, remaining-network and lost-coverage walk buffers). It can also write each scenario's surviving network as a standalone GTFS feed, so any other tool in this repository can run baseline-vs-scenario comparisons on ordinary feeds, and a screening mode (`--each-route`) evaluates the elimination of every route in one run, ranking routes by the riders and revenue hours at stake.
