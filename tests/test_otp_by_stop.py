"""Tests for otp_by_stop using the repo TIDES fixtures plus synthetic frames."""
...
import otp_by_stop as target  # noqa: E402
...
def test_run_writes_tables_and_runlog(tmp_path: Path) -> None:
    ...
    assert (tmp_path / target.OTP_BY_STOP_FILENAME).exists()
    assert (tmp_path / target.OTP_BY_STOP_ROUTE_DETAIL_FILENAME).exists()
    runlog = tmp_path / "otp_by_stop_runlog.txt"
